...
func _ready():
	r.connect("gold_changed", self, "_on_gold_changed")
	
func _on_gold_changed(new_resource_value: int):
	current_gold = new_resource_value
	toggleBuildRoom()
...
func toggleBuildRoom():
	$BuildRoom.disabled = current_selected_room_cost > current_gold
